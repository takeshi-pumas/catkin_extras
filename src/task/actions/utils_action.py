...
def add_collision_object(position = [0, 0 ,0], rotation = [0,0,0,1], dimensions = [0.05, 0.05, 0.05], object_name = "object", frame_id = "base_link"):
    # Crear un objeto de colisión
    collision_object = CollisionObject()
    collision_object.header.frame_id = frame_id  # Marco de referencia del robot
    collision_object.id = object_name

    primitive = SolidPrimitive()
    primitive.type = SolidPrimitive.BOX
    primitive.dimensions = dimensions  # Dimensiones de la caja (ancho, alto, profundidad)

    # Definir la posición y orientación del objeto
    pose = Pose()
    pose.position.x = position[0]  # Coordenada x
    pose.position.y = position[1]  # Coordenada y
    pose.position.z = position[2]  # Coordenada z
    pose.orientation.x = rotation[0]
    pose.orientation.y = rotation[1]
    pose.orientation.z = rotation[2]
    pose.orientation.w = rotation[3]  # Cuaternión de orientación

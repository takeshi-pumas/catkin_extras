...
class New_face(smach.State):
    ...
    def execute(self, userdata):
        ...
        if self.tries == 3:
            voice.talk ('I not undestood your name, lets keep going')
            name = 'someone'
...
class Introduce_guest(smach.State):
    ...
    def execute(self, userdata):
        ...
        takeshi_line = party.get_guest_description(name_face)                    
        drink = party.get_guest_drink(name_face)
...
if __name__ == '__main__':
    ...
    with sm:
        ...
        smach.StateMachine.add("GET_DRINK",         Get_drink(),    transitions={'failed': 'GET_DRINK',     'succ': 'LEAD_TO_LIVING_ROOM'})

        smach.StateMachine.add("LEAD_TO_LIVING_ROOM",   Lead_to_living_room(),  transitions={'failed': 'LEAD_TO_LIVING_ROOM',   'succ': 'FIND_SITTING_PLACE'})
        smach.StateMachine.add("FIND_SITTING_PLACE",    Find_sitting_place(),   transitions={'failed': 'FIND_SITTING_PLACE',    'succ': 'FIND_HOST'})
        smach.StateMachine.add("FIND_HOST",             Find_host(),            transitions={'failed': 'FIND_HOST',             'succ':'INTRODUCE_GUEST'})
        smach.StateMachine.add("INTRODUCE_GUEST",       Introduce_guest(),      transitions={'failed': 'INTRODUCE_GUEST',       'succ':'WAIT_PUSH_HAND',        'tries':'END'})

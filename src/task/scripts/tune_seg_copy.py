    ...
    if 32 <= keystroke and keystroke < 128:
        ...
        if key=='u': 
            ...
            for i in range(len(res.poses)):
                tf_man.getTF("head_rgbd_sensor_rgb_frame")
                position = [
                    res.poses[i].position.x,
                    res.poses[i].position.y,
                    res.poses[i].position.z
                ]
                print(position)
                tf_man.pub_static_tf(
                    pos=position,
                    rot=[0, 0, 0, 1],
                    ref="head_rgbd_sensor_rgb_frame",
                    point_name=res.names[i].data[4:] 
                )
                rospy.sleep(0.3)
                tf_man.change_ref_frame_tf(res.names[i].data[4:])
            debug_image = bridge.imgmsg_to_cv2(res.debug_image.image_msgs[0], desired_encoding="bgr8")
            cv2.imshow('YOLOv8 Result', debug_image)
            save_image(debug_image, name="yolo_result")
            ...
            best_name = class_names[max_conf_idx]

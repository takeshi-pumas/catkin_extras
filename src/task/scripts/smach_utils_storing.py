...
####################################################################################################################################################
# Only for StoringGloceries       
####################################################################################################################################################
def segment_region(region_name):
    bridge = CvBridge()
    pointcloud_msg = rospy.wait_for_message("/hsrb/head_rgbd_sensor/depth_registered/rectified_points", PointCloud2)
    # img_msg = rospy.wait_for_message('/hsrb/head_rgbd_sensor/rgb/image_raw', Image, timeout=10)
    ...
    try:
        ...
        if response.success:
            ...
            kernel = np.ones((13, 13), np.uint8)  # Define un kernel de 13x13 (ajustable)

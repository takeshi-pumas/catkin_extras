...
class Scan_restaurant(smach.State):
    ...
    def execute(self,userdata):
        ...
        xyz_wrt_robot= res_to_array(res)
        

        if res.success:return 'succ'
        else:
            if np.isnan(xyz_wrt_robot).any():
                print( ' no person found, are there obstacles?')
                hv= head.get_current_joint_values()
                hv[1]= -0.6
                head.go(hv)

                corrected=get_points_corrected()
                scaned_area =corrected['y'][200:280,280:360]
                if np.var(scaned_area) < 1e-9:  ### High var == occupied space in font,,, also not human
                    points= rgbd.get_points()       ### varianza baja en el ordn de e-10 .... espacio libre. Navega 
                    trans =[points[240,320][0],points[240,320][1],points[240,320][2]]
                    rot=[0,0,0,1]
                    #broadcaster.sendTransform(trans,rot, rospy.Time.now(), 'meta',"head_rgbd_sensor_link")#ROBOT REAL
                    broadcaster.sendTransform(trans,rot, rospy.Time.now(), 'meta',"head_rgbd_sensor_link")#ROBOT REAL
                    rospy.sleep(0.3)
                    xyz_map , quat = listener.lookupTransform('map', 'meta', rospy.Time(0)) #

                    
                    self.tries=0
                    print ('coords in dynamic tf meta ')
                    ...
                    static_transformStamped.header.stamp = rospy.Time.now()
                    static_transformStamped.header.frame_id = "map"
                    static_transformStamped.child_frame_id = "meta_st"
                    static_transformStamped.transform.translation.x = float(xyz_map[0])
                    static_transformStamped.transform.translation.y = float(xyz_map[1])
                    static_transformStamped.transform.translation.z = float(xyz_map[2])
                    #quat = tf.transformations.quaternion_from_euler(-euler[0],0,1.5)
                ...
                print( 'person found, not waving tho.') 
                return  'failed'
...
class Roam(smach.State):
    ...
    def execute(self,userdata):
        ...
        rospy.loginfo('STATE : ROAM')
        rospy.sleep(1)
        print('Check for obstacles  on intended pose')
        ...
        for i in range (10):
            try:
                trans,rot=listener.lookupTransform('map', 'meta_st', rospy.Time(0))  #Robot real
                rospy.sleep(10)
                print ('Going to trans', trans)
                move_base( trans[0],trans[1],0,50)
                return 'succ'

            except:
                print( 'no tf try',i,'of 10')
                rospy.sleep(1)
...
def init(node_name):
    global listener, broadcaster, tfBuffer, tf_static_broadcaster, scene, rgbd  , head,whole_body,arm,gripper  ,goal,navclient,clear_octo_client ,  detect_waving_client, class_names , bridge , base_vel_pub,takeshi_talk_pub, order

...
class Initial(smach.State):
    ...
    def execute(self, userdata):
        ...
        confirmation=['yes','no', 'takeshi yes', 'takeshi no','not','now' , 'nope']                                                                                          
    ...
    def execute(self, userdata):
        ...
        if res != None:
            ...
            if name == 'NO_FACE':
                ...
                if confirmation not in ['yes','jack','juice', 'yeah','takeshi yes']:
                    return 'unknown'
...
class Find_sitting_place(smach.State):
    ...
    def execute(self, userdata):
        ...
        seat_place=f'Place_face{self.tries}' #Looking in place 1
        
        head.to_tf(seat_place)
        ...
        if res == None:
            ...
            print("Place is: ",seat_place)
            _,guest = get_waiting_guests()
            head.set_named_target('neutral')
            rospy.sleep(0.8)
            #head.turn_base_gaze(tf=place)
            head.turn_base_gaze(tf=seat_place, to_gaze='arm_flex_link')
...
class Introduce_guest(smach.State):
    ...
    def execute(self, userdata):
        ...
        if self.tries==4:
            voice.talk('Sorry, there is no more places to look for host')
            self.tries=0
            return 'succ'
        seat_place=f'Place_face{self.tries}' #Looking in place 1
        ...
        voice.talk(f'looking for host on sit {self.tries}')
        res, _ = wait_for_face()
        print(res)

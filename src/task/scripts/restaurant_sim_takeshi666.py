...
def get_points_corrected():
    ...
    trans,rot=listener.lookupTransform('/map', '/head_rgbd_sensor_gazebo_frame', rospy.Time(0))  #Robot real
        ...
        if res.success:return 'succ'
            ...
            if np.isnan(xyz_wrt_robot).any():
                ...
                if np.var(scaned_area) < 1e-9:  ### High var == occupied space in font,,, also not human
                    ...
                    rot=[0,0,0,1]
                    broadcaster.sendTransform(trans,rot, rospy.Time.now(), 'meta',"head_rgbd_sensor_link")
                    rospy.sleep(0.3)
                    trans , rot = listener.lookupTransform('/map', '/meta', rospy.Time(0)) #
                    print (trans , rot)
                    self.tries=0
                    print ('coords in tf meta ')
...
class Roam(smach.State):
    ...
    def execute(self,userdata):
        ...
        for i in range (10):
            try:
                trans,rot=listener.lookupTransform('map', 'meta', rospy.Time(0))  #Robot real
                print ('Going to trans', trans)
                move_base( trans[0],trans[1],0,10)
                break

...
class Initial(smach.State):
    ...
    def execute(self, userdata):        
        ...
        global arm ,  hand_rgb , objs ,regions
        hand_rgb = HAND_RGB()
        ####KNOWLEDGE DATAFRAME 
...
class Scan_table(smach.State):
    ...
    def execute(self, userdata):
        ...
        def is_inside(x,y,z):return ((area_box[:,1].max() > y) and (area_box[:,1].min() < y)) and ((area_box[:,0].max() > x) and (area_box[0,0].min() < x)) and (self.pickup_plane_z<z)  
        for name in regions:
            in_region=[]
...
class Scan_shelf(smach.State):
    def __init__(self):
        ...
        self.top_shelf_height=  0.98 #0.8
    ...
    def execute(self, userdata):
        ...
        if "shelves_cats" in globals() and len(shelves_cats)==3:###SHELF ALREADY SCANNED
            ...
            y_range = np.arange(area[0,1]+0.20, area[1,1]-0.20, .13)
            x_range = np.arange(area[0,0]+0.20, area[1,0]-0.20, .13)

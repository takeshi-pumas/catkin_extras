...
TEST_F(SuctionControllerTest, SuctionSucceededTest) {
  // Set parameter for setting stub's result
  ros::NodeHandle nh;
  nh.setParam("/result_status", "SUCCEEDED");

  // Wait for exit hsrb_suction_controller node
  ros::Duration(2.0).sleep();
  ros::spinOnce();

  ASSERT_EQ(SuctionControllerTest::goal_list.size(), 2);
  ...
  // goal_list_.at(1): suction_off request
  EXPECT_EQ(SuctionControllerTest::goal_list.at(1).timeout, ros::Duration(0.0));
  EXPECT_FALSE(SuctionControllerTest::goal_list.at(1).suction_on.data);
  ...
  EXPECT_EQ(SuctionControllerTest::log_message_list.at(1),

...
###################################################################################
#TODO: move from rc24_receptionist.py:
class DescribePerson(smach.State):
    def __init__(self):
        smach.State.__init__(
            self,
            outcomes=['succ', 'failed'],
            input_keys=['actions', 'params'],
            output_keys=['actions', 'params']
        )            
        return 'failed'


    def execute(self, userdata):
        rospy.loginfo('STATE : Describe Person')

        try:
            if len(userdata.params) < 3:
                rospy.logerr('Not enough parameters for DescribePerson. Expected: image_path, description_id, prompt')
                return 'failed'

            image_path = userdata.params[0]
            description_id = userdata.params[1]
            prompt = userdata.params[2]

            rospy.loginfo(f"Describing person in image: {image_path} with ID: {description_id}")
            describe_person(image_path, description_id, prompt)

            # Wait for description file to appear
            description_file = f"{description_id}_person_description.txt"
            rospy.loginfo(f"Waiting for description file: {description_file}")

            timeout = rospy.Time.now() + rospy.Duration(10)
            while not os.path.exists(description_file):
                rospy.sleep(0.5)
                if rospy.Time.now() > timeout:
                    rospy.logerr("Timed out waiting for description file.")
                    return 'failed'

            with open(description_file, "r") as f:
                description = f.read().strip()
                rospy.loginfo(f"Description generated: {description}")
                talk(description)

            # Clean up
            userdata.actions.pop(0)
            for _ in range(3): userdata.params.pop(0)

            return 'succ'

        except Exception as e:
            rospy.logerr(f"Exception in DescribePerson state: {e}")
            return 'failed'

###################################################################################
#TODO: move from rc24_gpsr.py
class PostureActionRecognition(smach.State):
    def __init__(self):
        smach.State.__init__(
            self,
            outcomes=['succ', 'failed'],
    ...
    def recognize_action(self, keypoints):
        ...
        left_wrist = keypoints[7].pixel
        right_wrist = keypoints[4].pixel
        left_elbow = keypoints[6].pixel
        right_elbow = keypoints[3].pixel
        left_shoulder = keypoints[5].pixel
        right_shoulder = keypoints[2].pixel

        actions = []

        if left_wrist.x or left_wrist.y or left_elbow.x or left_elbow.y or left_shoulder.x or left_shoulder.y:
        ...
        if right_wrist.x or right_wrist.y or right_elbow.x or right_elbow.y or right_shoulder.x or right_shoulder.y:
    ...
    def recognize_posture(self, keypoints):
        ...
        left_hip = keypoints[12].pixel
        right_hip = keypoints[9].pixel
        left_knee = keypoints[13].pixel
        right_knee = keypoints[10].pixel
        left_shoulder = keypoints[5].pixel
        right_shoulder = keypoints[2].pixel

        if (left_hip.x == 0 or right_hip.x == 0 or left_knee.x == 0 or right_knee.x == 0 or
            left_shoulder.x == 0 or right_shoulder.x == 0):
            return None

        posture_angle_left = self.angle(left_shoulder, left_hip, left_knee)
        posture_angle_right = self.angle(right_shoulder, right_hip, right_knee)

        if 40 < posture_angle_left < 110 and 40 < posture_angle_right < 110:
            return "sitting"
    ...
    def execute(self, userdata):
        rospy.loginfo('STATE : Posture/Action Recognition')
        global keypoints
        keypoints = None  # reset if necessary

        try:
            action_item = userdata.params[0].lower().strip()
            rospy.loginfo(f"Looking for posture or gesture: {action_item}")

            gestures = {"raising left arm", "raising right arm", "pointing to the left", "pointing to the right"}
            postures = {"sitting", "standing", "laying"}

            for _ in range(10):
                if keypoints is not None and keypoints[11].pixel.x != 0.0:
                    if action_item in gestures:
                        detected = self.recognize_action(keypoints)
                        if action_item in detected:
                            talk(f"I see someone {action_item}")
                            userdata.actions.pop(0)
                            userdata.params.pop(0)
                            return 'succ'
                        else:
                            rospy.loginfo(f"Detected actions: {detected}")
                    elif action_item in postures:
                        detected = self.recognize_posture(keypoints)
                        if detected == action_item:
                            talk(f"The person is {detected}")
                            userdata.actions.pop(0)
                            userdata.params.pop(0)
                            return 'succ'
                        else:
                            rospy.loginfo(f"Detected posture: {detected}")
                rospy.sleep(0.5)

            talk("I could not detect the specified gesture or posture.")
            return 'failed'

        except Exception as e:
            rospy.logerr(f"Exception in PostureActionRecognition: {e}")
            return 'failed'

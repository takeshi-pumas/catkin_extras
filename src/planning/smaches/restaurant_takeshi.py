...
class Scan_space(smach.State):
    ...
    def execute(self,userdata):
        ...
        rospy.loginfo('State : SCAN_SPACE')
        head.set_named_target('neutral')
        head.go() 
        print(' 360 Deg turn to Map')
        for i in range(10):
            print('Iteration', i)
            #succ = move_base(0,0,0.64*i)
            succ = True
 
        if succ:
            print('Takeshi Mapping')
            return 'succ'
        else:
...
class Scan_restaurant(smach.State):
    ...
    def execute(self,userdata):
        ...
        for j in range(2):
            #move_base(0,0,1.57*j)
            print('Iteration', j, 'Searching for human')
            for i in range(3):
                print('Pose: ',i)
...
class Hri_take_order(smach.State):
    ...
    def execute(self,userdata):
        ...
        print ("-------------------------------") 
        print ("Please please please roslaunch restaurant_robot") 
        ...
        for word in wordlist:
            if word in ["apple", "orange", "banana", "strawberry", "lemon", "peach", "pear", "plum", "cookies", "pudding", "gelatin", "meat can", "coffee", "soup", "tuna can", "sugar", "mustard", "chips"]:
                ...
                succ = False
                print('Takeshi did not hear the order. Trying again')
...
class Hri_confirm_order(smach.State):
    ...
    def execute(self,userdata):
        ...
        voice_message=Voice()
        voice_message.sentence = 'Please confirm your order. Do you want a '+ order
...
def init(node_name):
    global listener, broadcaster, tfBuffer, tf_static_broadcaster, scene, rgbd  , head,whole_body,arm,gripper  ,goal,navclient,clear_octo_client , classify_client , class_names , bridge , base_vel_pub,takeshi_talk_pub, order
...
if __name__== '__main__':
    ...
    with sm:
        ...
        smach.StateMachine.add("SCAN_SPACE",        Scan_space(),       transitions = {'failed':'INITIAL',          'succ':'SCAN_RESTAURANT',   'tries':'INITIAL'}) 
        smach.StateMachine.add("SCAN_RESTAURANT",   Scan_restaurant(),  transitions = {'failed':'INITIAL',          'succ':'GOTO_TABLE',        'tries':'INITIAL'}) 
        ...
        smach.StateMachine.add("HRI_CONFIRM_ORDER", Hri_confirm_order(),   transitions = {'failed':'HRI_TAKE_ORDER',   'succ':'END',          'tries':'INITIAL'}) 

                               ...
                               nav_msgs::Path& resultPath)
{
    ...
    */
    ...
    if(map.data[goalCell] > BLACK_LEVEL || map.data[goalCell] < 0)
    {
        std::cout << "PathCalculator.-> Cannot calculate path: goal point is inside occupied space" << std::endl;
        return false;
    }
    if(map.data[startCell] > BLACK_LEVEL || map.data[startCell] < 0)
    {
                               ...
                               int*& resultWaveFront)
{
    ...
    for(size_t i=0; i< map.data.size(); i++)
        resultWaveFront[i] = ((map.data[i] >= 0 && map.data[i] < BLACK_LEVEL) ? 0 : 1);
                         ...
                         nav_msgs::Path& resultPath)
{
    ...
    for(int i=0; i< map.data.size(); i++)
    {
        isKnown[i] = map.data[i] > BLACK_LEVEL || map.data[i] < 0;
        g_values[i] = INT_MAX;
        f_values[i] = INT_MAX;
        previous[i] = -1;
        visited[i] = map.data[i] > BLACK_LEVEL || map.data[i] < 0;
    }
    for(int i=0; i< 8; i++)
        neighbors[i] = 0;
...
nav_msgs::OccupancyGrid PathCalculator::GrowObstacles(nav_msgs::OccupancyGrid& map, float growDist)
{
    ...
    */
    ...
    for(int i=startIdx; i < endIdx; i++)
        if(map.data[i] > BLACK_LEVEL) //Then, is an occupied cell

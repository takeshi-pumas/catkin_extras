...
ros::ServiceClient TakeshiManip::cltmoveitPosition;
...
bool TakeshiManip::moveitGoalPosition(std::string move_group, geometry_msgs::Pose pose){
  /*
  option of move_group:
  -whole_body
  -whole_body_light
  -whole_body_weighted
  */
  moveit_services::moveitPosition srv;
  srv.request.pose = pose;
  srv.request.move_group = move_group;
  if (cltmoveitPosition.call(srv))
  {
    std::cout<<"position reachead"<<std::endl;
    return true;
  }
  else
  {
    ROS_ERROR("ERROR position not reachead");
    return false;
  }
}

bool TakeshiManip::moveitGoalPosition(std::string move_group, float x, float y, float z, float roll, float pitch, float yaw){
  ...
  return TakeshiManip::moveitGoalPosition(move_group, pose);

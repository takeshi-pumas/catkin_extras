...
class LocationServer:
    ...
    def load_file(self, file_name):
        ...
        try:
            ...
            file_content = {}
            self.write_yaml(file_name, file_content)
        return file_content
    ...
    def publish_tf_knowledge(self, knowledge):
        for place_name, place_data in knowledge["places"].items():
            ...
            quat = quaternion_from_euler(0, 0, place_data["location"]["theta"])
    ...
    def handle_location_add(self, req):
        ...
        if succ:
            ...
            self.known_locations[location_name] = [
                {'x': round(pose[0], 2)}, 
                {'y': round(pose[1], 2)}, 
                {'theta': round(euler_from_quaternion(quat)[2], 2)},
    ...
    def handle_knowledge_place_add(self, req):
        ...
        succ, pose, quat = self.get_robot_location()
        # if succ:
        #     if "places" not in self.knowledge:
        #         rospy.logwarn("'places' key not found in knowledge. Creating an empty structure.")
        #         self.knowledge["places"] = {}
        if succ:
            # Asegurar que "places" existe y es un diccionario
            if "places" not in self.knowledge or not isinstance(self.knowledge["places"], dict):
                rospy.logwarn("'places' key not found or invalid. Creating an empty dictionary.")
                self.knowledge["places"] = {}
            
            place_number = len(self.knowledge["places"]) +1
            place_name = f"Place_{place_number}"

            self.knowledge["places"][place_name] = {
                "location": {"x": round(pose[0], 2), 
                             "y": round(pose[1], 2), 
                             "theta": round(euler_from_quaternion(quat)[2], 2)},
                "occupied": ''
            }

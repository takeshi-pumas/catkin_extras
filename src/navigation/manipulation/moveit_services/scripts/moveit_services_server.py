...
class TakeshiMoveIt():
	...
	def transform_to_reference_frame(self, pose, origin_ref_frame):
		try:
			(trans,rot) = self.listener.lookupTransform(self.reference_frame, origin_ref_frame, rospy.Time(0))
			print "Original Pose:"
			print pose
			print"#########################"
			print"tranfrom"
...
def handle_moveit_position(req):
	...
	return robot. go_to_pose_goal(req.move_group,robot.transform_to_reference_frame(req.pose,"/base_link"))
...
def main():
	...
	s_place = rospy.Service('/hsrb/manipulation/moveit/place', moveitPosition, handle_place)

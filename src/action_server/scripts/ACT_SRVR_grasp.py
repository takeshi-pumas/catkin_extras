...
class GraspingStateMachine:
    def __init__(self):
        ...
        self.grasp_approach = "frontal" #above

        # Moveit setup
        #self.arm.allow_replanning(True)
        self.whole_body.allow_replanning(True)
        self.whole_body.set_num_planning_attempts(5)
        self.whole_body.set_planning_time(7.0)
        #self.arm.set_num_planning_attempts(5)
        #self.arm.set_planning_time(7.0)
        self.whole_body.set_workspace([-2.0, -2.0, 2.0, 2.0])
    ...
    def calculate_frontal_approach(self, target_position = [0.0, 0.0, 0.0]):
        ...
        # approach_positon = np.array([transformed_object_point.point.x - 0.05,
        #                               transformed_object_point.point.y,
        #                               transformed_object_point.point.z])
        approach_pose = Pose()
        ...
        quat = quaternion_from_euler(np.pi, -np.pi/2, 0)
        approach_pose.orientation = Quaternion(*quat)
        #approach_pose.orientation = Quaternion(0.0, 0.7071, 0.0, 0.7071)
    ...
    def calculate_above_approach(self, target_position = [0.0, 0.0, 0.0]):
        ...
        # approach_posiiton = np.array([transformed_object_point.point.x,
        #                               transformed_object_point.point.y,
        #                               transformed_object_point.point.z + 0.05])
        approach_pose = Pose()
    ...
    def grasp(self, userdata):
        ...
        #gripper.close(0.04)
        # rospy.sleep(1)
        self.gripper.close(0.3)
        self.attach_object()
        return 'success'

    def retreat(self, userdata):
        pose_goal = self.target_pose
        pose_goal.position.x -= 0.07
        pose_goal.position.z += 0.07
        self.move_to_pose(self.whole_body, pose_goal)  # Retirarse a una posición segura
        return 'success'
    ...
    def attach_object(self):
        ...
        eef_link = self.whole_body.get_end_effector_link()

        self.scene.attach_box(eef_link, "objeto", touch_links=touch_links)

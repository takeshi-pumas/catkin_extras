...
'''class HEAD():
    ...
    def _gaze_point(self):
    ###Moves head to make center point of rgbd image to coordinates w.r.t.map
        succ,trans,_ = self._tf_man.getTF(reference=self._reference,target=self._cam)
        #maybe this is not needed anymore
        #rospy.sleep(0.3)
        succ,_,rot = self._tf_man.getTF(reference=self._reference, target=self._base)

        _,_, th_rob = self._tf_man.quat_to_eu(rot)
    ...
    def to_tf(self, target_frame='None'):
        if target_frame != 'None':
            succ, xyz, _ = self._tf_man.getTF(target=target_frame)
            #rospy.sleep(0.5)
            tries = 1
            #not succ --> tf no encontrada
            while (not succ) and (tries <= 5):
                tries += 1
                print(tries)
                succ, xyz, _ = self._tf_man.getTF(target=target_frame)
                rospy.sleep(0.5)
    ...
    def turn_base_gaze(self,tf='None', align = 'bl'):
        base = OMNIBASE()
        succ = False
        THRESHOLD = 0.05
        ref = self._base
        ...
        if tf != 'None':
            ...
            target_frame = 'gaze'
            self._tf_man.pubTF(pos=[self._x,self._y,self._z], TF_name='gaze')
            rospy.sleep(0.8)
        while not succ:
            done,xyz,_=self._tf_man.getTF(target=target_frame, reference=ref)
            eT = 0
            if done:
                eT = np.arctan2(xyz[1],xyz[0])
                succ = abs(eT) < THRESHOLD 

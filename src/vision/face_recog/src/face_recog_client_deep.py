...
class FaceRecognitionClient:
    ...
    def recognize_face(self):
        ...
        try:
            ...
            if len(response.bounding_boxes) != 0:
                bb = response.bounding_boxes[0]
                start_point = (bb.xmin, bb.ymin)
                end_point = (bb.xmax, bb.ymax)
            if response.name_response:
                print(f"✅ Persona reconocida: {response.name_response[0].data}")
                bb = cv2.rectangle(frame, start_point, end_point, (255,0,0), 2)

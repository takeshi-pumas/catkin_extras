		...
		while True:
		    ...
		    if datum.poseKeypoints is not None:
		    	...
		    	for i in range(dataout.shape[0]):
		    		f,_ = distance_to_people(dataout[i],dataPC)
		    		if f==1:
		    			c_general=1
		    			break
		    	if c_general==0:
		    		...
		    		for i in range(dataout.shape[0]):
		    			...
		    			if detect_drinking(dataout[i]):
		    				f,d = distance_to_people(dataout[i],dataPC)
		    				if f==1:
		    					conteo_sin_bebida[i]=0
		    					cnt_normal+=1
		    				draw_text_bkgn(image,text="Person "+str(i)+":  DIST:"+str(d),pos=(int(dataout[i,0,0]), int(dataout[i,0,1])-40),
